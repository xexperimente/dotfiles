...
function state.set_mode()
	...
	state.mode_group = 'SnacksPickerCmd'
	text = fmt(' %s ', mode_name)

	vim.g.stl_mode = fmt(hi_pattern, state.mode_group, text)
end

function state.position() vim.g.stl_position = fmt(hi_pattern, state.mode_group, ' %3l:%-2c ') end

function state.restore_active()
	if state.transition == 'done' then return end
...
command('StlDiagnostics', function(input)
	...
	if input.bang then vim.cmd('redrawstatus') end
...
autocmd('FileType', {
	...
	callback = function(event)
		...
		autocmd('BufUnload', {
			buffer = event.buf,
			callback = function() vim.w.stl_style = 'default' end,
		})
	end,
})
...
autocmd('LspAttach', {
	...
	callback = function(event)
		...
		if client:supports_method('textDocument/diagnostics') then state.show_diagnostic = true end
	end,
})
...
autocmd('WinEnter', {
	...
	callback = function()
		...
		if winid and vim.api.nvim_win_is_valid(winid) then vim.wo[winid].statusline = state.inactive_pattern end
